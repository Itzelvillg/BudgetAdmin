  ...
  return (
    <>
      <div className="flex justify-center gap-5 items-center ">
        <img src="presupuesto.png" alt="quick budget" className="w-12 h-12 " />
        <h1 className="text-4xl font-black text-blue-700 ">Quick Budget Recap</h1>

      </div>
      < div>
        <p className="text-2xl font-bold text-gray-700">Budget: <span className="text-1xl border-b-2 rounded border-b-slate-300">{formatCurrency(initialBudget)}</span></p>
        <div className="flex items-center justify-between mt-5">
          ...
          <div className="flex flex-col items-center  p-4 rounded">
            ...
            <p className={`text-3xl font-bold   ${getSavings() > 0 ? 'text-green-500' : 'text-gray-700'} `}>{formatCurrency(getSavings())}</p>
          </div>
          <div className="flex flex-col items-center p-4 rounded">
            <p className="text-gray-700">Spent so far:</p>
            <p className="text-3xl font-bold text-gray-700">{formatCurrency(getTotalExpenses())}</p>
          </div>
        </div>
